...
async function sendHostEmail(visit, token) {
  const host = process.env.BASE_URL || 'http://localhost:3001';
...
router.post('/:id/activate', async (req, res) => {
  ...
  try {
    ...
    if (useBadge === 'rfid') {
      ...
      // 1. Check if client requested a specific valid tag and it is available
      if (rfid_tag && CANONICAL_TAGS.includes(rfid_tag)) {
        const dbRow = dbMap[rfid_tag];
        if (!dbRow || dbRow.available === true) {
          assignedTag = rfid_tag;
        }
      }

      // 2. Fallback to auto-assignment if no valid/available tag was specified
      if (!assignedTag) {
        for (let i = 1; i <= 10; i++) {
...
router.post('/:id/checkout', async (req, res) => {
  ...
  try {
    const { rows } = await db.query(
      `SELECT v.in_time, v.out_time, v.duration_minutes, v.rfid_tag, v.badge_type, v.name, v.status, h.name as host_name
       FROM visits v
       LEFT JOIN hosts h ON v.host_id = h.id
       WHERE v.id=$1`,
      [id]
...
// GET /api/visits/search - Search active visit for RFID/Phone/Email
router.get('/search', async (req, res) => {
  let { identifier } = req.query;

  if (!identifier) {
    return res.status(400).json({ success: false, error: 'Search identifier required' });
  }

  identifier = identifier.trim().toUpperCase();

  function detectType(val) {
    if (/^VISITOR-\d+$/i.test(val)) return 'RFID';
    if (/^\d{10}$/.test(val)) return 'MOBILE';
  ...
  try {
    ...
    if (type === 'RFID') {
      ...
      const { rows } = await db.query(
        `SELECT * FROM visits
         WHERE (phone = $1 OR RIGHT(REGEXP_REPLACE(phone, '\\D', '', 'g'), 10) = $2)
           AND out_time IS NULL 
           AND (status = 'active' OR status = 'ACTIVE') 
         LIMIT 1`,
        [identifier, last10]
    ...
    if (!visit) {
      return res.status(404).json({ success: false, error: 'No active visit found' });
    }
    ...
    console.error('[GET Search API Error]', err);
    res.status(500).json({ success: false, error: 'Search failed' });
  }
});

// POST /api/visits/search - Strict checkout search validation based on accessMode
router.post('/search', async (req, res) => {
  const { identifier, accessMode } = req.body;

  if (!identifier) {
    return res.status(400).json({ ok: false, error: 'Search identifier required' });
  }
  ...
  function detectType(val) {
    ...
    if (/^\d{10}$/.test(val)) return 'MOBILE';
    if (/^\S+@\S+\.\S+$/.test(val)) return 'EMAIL';
    if (val.startsWith('QR-') || val.length > 20) return 'QR';
    return 'UNKNOWN';
  }
  ...
  try {
    ...
    if (type === 'RFID') {
      queryStr = `
        ...
        FROM visits v
        LEFT JOIN hosts h ON v.host_id = h.id
        WHERE v.rfid_tag = $1 AND (v.status = 'active' OR v.status = 'ACTIVE')
        LIMIT 1`;
    } else if (type === 'MOBILE') {
      ...
      queryStr = `
        ...
        FROM visits v
        LEFT JOIN hosts h ON v.host_id = h.id
        WHERE (v.phone = $1 OR RIGHT(REGEXP_REPLACE(v.phone, '\\D', '', 'g'), 10) = $2)
          AND (v.status = 'active' OR v.status = 'ACTIVE')
        LIMIT 1`;
      queryParams = [input, last10];
    } else if (type === 'EMAIL') {
      queryStr = `
        SELECT v.*, h.name as host
        FROM visits v
        LEFT JOIN hosts h ON v.host_id = h.id
        WHERE LOWER(v.email) = LOWER($1) AND (v.status = 'active' OR v.status = 'ACTIVE')
        LIMIT 1`;
    } else if (type === 'QR') {
      queryStr = `
        SELECT v.*, h.name as host
        FROM visits v
        LEFT JOIN hosts h ON v.host_id = h.id
        WHERE v.qr_code = $1 AND (v.status = 'active' OR v.status = 'ACTIVE')
        LIMIT 1`;
    } else {
      return res.status(404).json({ ok: false, error: 'No active visit' });
    }
    ...
    if (accessMode === 'QR' && type === 'RFID') {
      return res.status(400).json({ ok: false, error: 'RFID not allowed in QR mode' });
    }

    if (accessMode === 'RFID' && type === 'QR') {
      return res.status(400).json({ ok: false, error: 'QR not allowed in RFID mode' });
    }
...
// Duplicate search and active-lookup endpoints removed to keep routes clean.
...
// POST /api/visits/:id/pdf - Generate and download PDF report from UI in-memory data
  ...
  <div class="card">
    ...
    <div class="card-body">
      ...
      <div class="data-box">
        ...
        <div class="dr"><div class="dv"><span class="badge badge-green">${iconCheck} Signed</span></div></div>
      </div>
    </div>
  </div>
</body>
...
// GET /api/visits/:id/badge - Badge lookup page (for QR scan)
