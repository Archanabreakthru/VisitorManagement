...
(function initBridge(){
  ...
  // POST /api/visits/search — find an active visit by phone, email, or rfid (Strict checking)
  window.apiLookupActiveVisit = async function(query){
    const res = await request(`/visits/search`, {
      method: 'POST',
      body: { identifier: query, accessMode: window.accessMode }
    });
